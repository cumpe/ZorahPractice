...
public class PartyJoinArgument extends PluginCommandArgument {
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        ...
        if (!ManagerHandler.getPartyManager().getParties().isEmpty()) {
            if (args.length == 0 || args[0].equals("")) {
                for (Party party : ManagerHandler.getPartyManager().getParties().values()) {
                    completions.add(party.getLeader().getName());
                }
            }
            else {
                for (Party party : ManagerHandler.getPartyManager().getParties().values()) {
                    if (party.getLeader().getName().toLowerCase().startsWith(args[0].toLowerCase())) {
                        completions.add(party.getLeader().getName());
                    }
                }
            }
